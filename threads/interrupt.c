...
/* FUNCTION을 호출하는 게이트를 생성합니다.
   ...
   이 게이트는 서술자 권한 레벨 DPL을 가지며, 
   이는 프로세서가 DPL 또는 더 낮은 번호의 링에서 
   의도적으로 호출될 수 있음을 의미합니다. 
   실제로 DPL==3은 사용자 모드에서 게이트로의 호출을 허용하고, 
   DPL==0은 그러한 호출을 방지합니다. 
   사용자 모드에서 발생하는 오류나 예외는 
   DPL==0인 게이트가 호출되도록 합니다.

   TYPE은 인터럽트 게이트에 대해 14 또는 
   트랩 게이트에 대해 15 중 하나여야 합니다. 
   차이점은 인터럽트 게이트에 진입하면 인터럽트가 비활성화되지만, 
   트랩 게이트에 진입할 때는 비활성화되지 않는다는 것입니다. 
   논의는 [IA32-v3a] 섹션 5.12.1.2 
   "예외 또는 인터럽트 핸들러 절차에 의한 플래그 사용"을 참조하세요. */

struct gate {
	unsigned off_15_0 : 16;   // 세그먼트 내 오프셋의 하위 16비트// low 16 bits of offset in segment
	unsigned ss : 16;         // 세그먼트 선택자// segment selector
	unsigned ist : 3;         // 인터럽트/트랩 게이트의 경우 0// # args, 0 for interrupt/trap gates
	unsigned rsv1 : 5;        // 예약됨(0이어야 한다고 생각됩니다)// reserved(should be zero I guess)
	unsigned type : 4;        // 타입(STS_{TG,IG32,TG32})// type(STS_{TG,IG32,TG32})
	unsigned s : 1;           // 시스템용이어야 하며 0이어야 함// must be 0 (system)
	unsigned dpl : 2;         // 디스크립터의 새로운 권한 레벨을 의미// descriptor(meaning new) privilege level
	unsigned p : 1;           // 존재 여부// Present
	unsigned off_31_16 : 16;  // 세그먼트 내 오프셋의 상위 비트// high bits of offset in segment
	uint32_t off_32_63;
	uint32_t rsv2;
};
...
/* 인터럽트 디스크립터 테이블 (IDT). 형식은 CPU에 의해 고정됩니다. 
   [IA32-v3a] 섹션 5.10 "인터럽트 디스크립터 테이블(IDT)", 5.11 "IDT 디스크립터",
   5.12.1.2 "예외 또는 인터럽트 핸들러 절차에 의한 플래그 사용"을 참조하세요. */
...
/* External interrupts are those generated by devices outside the
   ...
   interrupt returns. */
/* 	외부 인터럽트는 CPU 외부의 장치들, 
	예를 들어 타이머에 의해 생성됩니다. 
   	외부 인터럽트는 인터럽트가 꺼진 상태에서 실행되므로, 
	중첩되거나 선점되지 않습니다.
   	외부 인터럽트의 핸들러는 대기 상태에 들어갈 수 없지만, 
	인터럽트가 반환되기 직전에 
   	새 프로세스의 스케줄링을 요청하는 
	intr_yield_on_return()을 호출할 수 있습니다. */
static bool in_external_intr;   /* 외부 인터럽트를 처리 중인가요? *//* Are we processing an external interrupt? */
static bool yield_on_return;    /* 인터럽트 반환 시에 실행 흐름을 양보해야 하나요? *//* Should we yield on interrupt return? */

/* Programmable Interrupt Controller helpers. */
/* 프로그래머블 인터럽트 컨트롤러 보조 도구. */
...
intr_get_level (void) {
	...
	/* 	프로세서 스택에 플래그 레지스터를 푸시한 다음,
   		스택에서 값을 팝하여 `flags`에 저장합니다. 
		[IA32-v2b]의 "PUSHF" 및 
		"POP"과 [IA32-v3a] 5.8.1 "Masking Maskable Hardware
   		Interrupts"를 참조하세요. */
...
intr_disable (void) {
	...
	/* Disable interrupts by clearing the interrupt flag.
	   ...
	   Hardware Interrupts". */
	/* 	인터럽트 플래그를 클리어함으로써 인터럽트를 비활성화합니다.
		[IA32-v2b]의 "CLI" 및 
		[IA32-v3a] 5.8.1 "Masking Maskable Hardware Interrupts"를
		참조하세요. */
...
/* 	인터럽트 벡터 번호 VEC_NO에 핸들러 HANDLER를 등록하여 
	디스크립터 권한 레벨 DPL로 호출되도록 설정합니다.
   	디버깅 목적으로 인터럽트의 이름을 NAME으로 지정합니다.
   	인터럽트 핸들러는 인터럽트 상태 LEVEL로 설정된 상태에서 호출됩니다. */
static void
...
/* 	인터럽트 벡터 번호 VEC_NO에 핸들러 HANDLER를 등록하여 
	디스크립터 권한 레벨 DPL로 호출하도록 설정합니다.
   	디버깅 목적으로 인터럽트의 이름을 NAME으로 명명합니다.
   	인터럽트 핸들러는 인터럽트 상태 LEVEL로 설정된 상태에서 호출됩니다. */
void
...
/* Registers internal interrupt VEC_NO to invoke HANDLER, which
   ...
   discussion. */
/* 내부 인터럽트 VEC_NO를 등록하여 핸들러 HANDLER를 호출하고, 디버깅 목적으로 이름 NAME을 지정합니다.
   인터럽트 핸들러는 인터럽트 상태 LEVEL로 호출됩니다.

   핸들러는 디스크립터 권한 레벨 DPL을 가지며, 이는 프로세서가 DPL 또는 그보다 낮은 번호의 링에서 의도적으로 호출될 수 있음을 의미합니다.
   실제로, DPL==3은 사용자 모드에서 인터럽트를 호출할 수 있게 하고, DPL==0은 그러한 호출을 방지합니다.
   사용자 모드에서 발생하는 오류나 예외는 DPL==0인 인터럽트가 호출되도록 합니다.
   추가 논의는 [IA32-v3a] 섹션 4.5 "권한 레벨"과 4.8.1.1 "비일치 코드 세그먼트 접근"을 참조하세요. */
void
...
/* During processing of an external interrupt, directs the
   ...
   time. */
/* 외부 인터럽트 처리 중, 인터럽트로부터 반환되기 직전에 새 프로세스로 양보하도록 인터럽트 핸들러에 지시합니다.
   다른 시간에는 호출될 수 없습니다. */
void
...
/* 주어진 IRQ에 대해 PIC에 인터럽트 종료 신호를 보냅니다.
   IRQ를 확인하지 않으면 다시 전달받지 못하므로 이는 중요합니다. */
static void
...
/* 모든 인터럽트, 장애, 예외에 대한 핸들러입니다.
   이 함수는 어셈블리 언어 인터럽트 스텁(intr-stubs.S 파일에 위치)에 
   의해 호출됩니다.
   FRAME은 인터럽트와 인터럽트 당한 스레드의 레지스터를 설명합니다. */
void
intr_handler (struct intr_frame *frame) {
	...
	/* 	외부 인터럽트는 특별합니다.
   		한 번에 하나만 처리할 수 있으며
		(따라서 인터럽트는 꺼져 있어야 합니다),
   		PIC에서 이를 확인해야 합니다(아래 참조).
   		외부 인터럽트 핸들러는 대기 상태에 들어갈 수 없습니다. */
